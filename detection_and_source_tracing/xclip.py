...
def main(args):
    ...
    if args.train == "True":
        ...
        processor = AutoProcessor.from_pretrained("microsoft/xclip-large-patch14", **hf_kwargs(args))
        xclip = AutoModel.from_pretrained("microsoft/xclip-large-patch14", **hf_kwargs(args))
        binary_cla = models.BinaryClassifier(768, num_classes=args.label_number)
        video_cls = models.VideoClassifier(xclip, binary_cla)
        ...
        binary_cla = models.BinaryClassifier(768, num_classes=args.label_number)
        video_cls = models.VideoClassifier(xclip, binary_cla)
        video_cls.load_state_dict(torch.load(args.load_pre_trained_model_state, map_location="cpu"))
        video_cls = video_cls.to(device)
        print("load model...")
